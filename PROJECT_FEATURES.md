...
아래 내용은 운영 장애 이력이 아니라, 현재 코드에 반영된 안정성 대응 항목입니다.

1. API Prefix 정합성
- 프론트는 환경변수 VITE_API_BASE_URL 기준 호출, 백엔드는 /api/v1 prefix 사용
- prefix 불일치 시 404를 방지하도록 문서/환경변수 표준값을 고정

2. 대용량 업로드 보호
- 업로드는 청크 단위 저장, 파일당 1GB 제한 적용
- 제한 초과 시 저장 파일 정리 후 예외 반환

3. 실패 파일 재처리 안전장치
- retry API는 FAILED 상태 파일만 허용
- 원본 파일 누락 시 재처리 중단 및 명확한 오류 반환

4. Notion OAuth 콜백 방어 로직
- state(UUID) 검증
- 저장 실패 시 rollback
- 예외 코드 분리로 일관된 실패 응답 보장
